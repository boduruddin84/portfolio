const Stacks = () => {
  const stacks = [
    ...
    { name: 'Express.js', icon: 'dns' }, // updated
    ...
    <section className="space-y-12" id="technologies">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {stacks.map((stack) => (
          <div
            key={stack.name}
            className="bg-surface-container-low border border-outline-variant/30 p-8 rounded-2xl flex flex-col items-center justify-center gap-4 hover:border-primary transition-colors group"
          >
